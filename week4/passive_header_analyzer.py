...
class PassiveHeaderAnalyzer:
    def __init__(self, session=None):
        ...
        self.session.headers.update({
            "User-Agent": "PassiveHeaderAnalyzer/1.0",
    ...
    def generate_header_report(self, output_file: str = "header_analysis.json"):
    ...
    def print_summary(self):
        ...
        print("\n" + "="*60)
        print("🔍 PASSIVE HEADER ANALYSIS SUMMARY")
        print("="*60)
        ...
        # Breakdown
        print(f"✅ Security Headers Present: {len(self.findings['security_headers'])}")
        print(f"❌ Missing Headers: {len(self.findings['missing_headers'])}")
        print(f"⚠️  Misconfigured Headers: {len(self.findings['misconfigured_headers'])}")
        print(f"🔍 Information Disclosure: {len(self.findings['information_disclosure'])}")
        print(f"🍪 Cookie Issues: {len(self.findings['cookies_analysis'])}")
        print(f"🖥️  Server Analysis: {len(self.findings['server_analysis'])}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output", default="header_analysis.json", help="Output file")
